...
print("Smooth distribution over 10 first time steps")
print(smooth_dist[1:10,1:10])
print("Filtered distribution over 10 first time steps")
print(filtered_dist_norm[1:10,1:10])
...
```{r}
# Function to calculate the Smooth, Filtered and Path accuracy given a HMM, a number of time steps and a seed. Also returns the probability distribution for both the Filtered and Smooth calculations. Set time steps constent and alter seed or vice versa for useful insights.
simulationStepComputation <- function(hmm, time_steps, seed) {
  ...
  # Smooth (_new is added to the calculated variables to differ from the previous task variables).
...
accs_df_2 <- data.frame(Smooth=numeric(0), Filtered = numeric(0), Path = numeric(0))
...
for(i in 1:number_of_seeds) {
  simulation_step_2 <- simulationStepComputation(hmm = HMM, time_steps = 200, seed = i)
...
accumulated_mean_df <- data.frame(t(rbind(accumulated_mean_filtered, accumulated_mean_smooth, time_steps =(1:300))))
